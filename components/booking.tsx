                  ...
                  <div className="rounded-2xl border border-border bg-card shadow-sm shadow-foreground/[0.03] p-8">
                    ...
                    {step === "done" && (
                      <div className="flex flex-col items-center gap-4 py-12 text-center">
                        ...
                        <p className="text-muted-foreground">You'll receive a calendar invite at {form.email}</p>
                        <button onClick={() => { setStep("calendar"); setSelectedSlot(null); setForm({ name: "", email: "", message: "" }) }}
                    ...
                    {step === "calendar" && (
                      <div>
                        <div className="flex items-center justify-between mb-6">
                          <button onClick={prevMonth} className="p-2 rounded-lg hover:bg-secondary"><ChevronLeft className="h-4 w-4" /></button>
                          <h3 className="font-semibold">{monthNames[month]} {year}</h3>
                          <button onClick={nextMonth} className="p-2 rounded-lg hover:bg-secondary"><ChevronRight className="h-4 w-4" /></button>
                        </div>
                            ...
                            return (
                              <button
                                ...
                                className={`aspect-square rounded-lg text-sm font-medium transition-colors
                                  ${disabled ? "text-muted-foreground/40 cursor-not-allowed" : "hover:bg-primary hover:text-primary-foreground cursor-pointer"}
                                  ${selectedDate === `${year}-${String(month+1).padStart(2,'0')}-${String(day).padStart(2,'0')}` ? "bg-primary text-primary-foreground" : ""}
                                `}
                              >
                                {day}
                              </button>
                        ...
                        <p className="mt-4 text-xs text-muted-foreground text-center">Available Monday–Friday, 9:00–18:00 Sydney time</p>
                      </div>
                    )}
                    ...
                    {step === "slots" && (
                      <div>
                        <button onClick={() => setStep("calendar")} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-6">
                          <ChevronLeft className="h-4 w-4" /> Back
                        </button>
                        <h3 className="font-semibold mb-4 flex items-center gap-2"><Clock className="h-4 w-4 text-primary" /> Available times for {selectedDate}</h3>
                        {loadingSlots ? (
                            ...
                            {slots.map(slot => (
                              <button
                                ...
                                className={`rounded-xl border px-4 py-3 text-sm font-medium transition-colors
                                  ${slot.available ? "border-border hover:border-primary hover:bg-primary/5" : "border-border/50 text-muted-foreground/40 cursor-not-allowed line-through"}
                                `}
                              >
                                {slot.label}
                              </button>
                    ...
                    {step === "form" && (
                      <div>
                        <button onClick={() => setStep("slots")} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-6">
                          <ChevronLeft className="h-4 w-4" /> Back
                        </button>
                        ...
                        <div className="flex flex-col gap-4">
                          ...
                          <textarea placeholder="Message (optional)" value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
                            rows={3} className="resize-none rounded-xl border border-border bg-background px-4 py-3 text-sm focus:border-primary focus:outline-none" />
